let tmi,fs,db,dbeco,dbcommands,dbtime,opluted,dbplayers,dbsixletterwords,dbsevenletterwords,dbeightletterwords,BOT_USERNAME,OAUTH_TOKEN,CHANNEL_NAME,BLOCKED_WORDS,TOKEN,ClientID,UserID,fetch,uuidV4,count;_684‍.w("tmi.js",[["default",["tmi"],function(v){tmi=v}]]);_684‍.w("fs",[["default",["fs"],function(v){fs=v}]]);_684‍.w("../db.json",[["default",["db"],function(v){db=v}]]);_684‍.w("../dbeco.json",[["default",["dbeco"],function(v){dbeco=v}]]);_684‍.w("../dbcommands.json",[["default",["dbcommands"],function(v){dbcommands=v}]]);_684‍.w("../dbtime.json",[["default",["dbtime"],function(v){dbtime=v}]]);_684‍.w("../opluted.json",[["default",["opluted"],function(v){opluted=v}]]);_684‍.w("../dbplayers.json",[["default",["dbplayers"],function(v){dbplayers=v}]]);_684‍.w("../dbsixletterwords.json",[["default",["dbsixletterwords"],function(v){dbsixletterwords=v}]]);_684‍.w("../dbsevenletterwords.json",[["default",["dbsevenletterwords"],function(v){dbsevenletterwords=v}]]);_684‍.w("../dbeightletterwords.json",[["default",["dbeightletterwords"],function(v){dbeightletterwords=v}]]);_684‍.w("./constants",[["BOT_USERNAME",["BOT_USERNAME"],function(v){BOT_USERNAME=v}],["OAUTH_TOKEN",["OAUTH_TOKEN"],function(v){OAUTH_TOKEN=v}],["CHANNEL_NAME",["CHANNEL_NAME"],function(v){CHANNEL_NAME=v}],["BLOCKED_WORDS",["BLOCKED_WORDS"],function(v){BLOCKED_WORDS=v}],["TOKEN",["TOKEN"],function(v){TOKEN=v}],["ClientID",["ClientID"],function(v){ClientID=v}],["UserID",["UserID"],function(v){UserID=v}]]);_684‍.w("node-fetch",[["default",["fetch"],function(v){fetch=v}]]);_684‍.w("uuid",[["v4",["uuidV4"],function(v){uuidV4=v}]]);_684‍.w("console",[["count",["count"],function(v){count=v}]]);
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (userstate.username == 'streamelements' || userstate.username == 'nightbot') {
    ...
    } else if (message.toLowerCase().includes(`u w usta`) || message.toLowerCase().includes(`modułu`) || message.toLowerCase().includes(`tytułu`) || message.toLowerCase().includes(`uchu w uchu`) || message.toLowerCase().includes(`u wuj`)) {
        return;
        ...
        if (userstate.mod && shouldSendMessage) {
            ...
            if (listaSpier.includes(userstate.username)) {
                ...
                //client.say(channel, "/timeout ${userstate.username} 600")
                ...
                //client.say(channel, "/timeout ${userstate.username} 20")
...
function delayToBoks() {
    ...
    _684‍.g.console.log("min do startu: " + (r / 60 / 1000).toFixed(0))
}
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (blockadeQ) {
        ...
        if (userstate.mod || userstate.username == 'itzmaxinho') {
            ...
            if (message.toLowerCase().startsWith('!swap')) {
                ...
                _684‍.g.console.log("to jest command: " + command)
                _684‍.g.console.log("to jest argument: " + argument)
                _684‍.g.console.log("to jest rest: " + rest)
                _684‍.g.console.log("to jest rest2: " + rest2)
                _684‍.g.console.log("to jest rest3: " + rest3)
                ...
                if (rest.toLowerCase() === "x") {
                    ...
                    (async () => {
                        try {
                            ...
                            _684‍.g.console.log("chatters: " + chatters)
    ...
    function changeAmountOfGamesById(viewer, games) {
        ...
        _684‍.g.console.log("Liczba gierek do zmiany: " + games)
        _684‍.g.console.log("Zmiana liczby gierek dla: " + viewer)
    ...
    function addPlayerToQ(argument, position, gamesAmount) {
        ...
        _684‍.g.console.log("ADDING PLAYER TO position: " + ++position)
        return player
    }
    ...
    function swapPlayersById(id1, id2) {
        ...
        _684‍.g.console.log(index1)
        _684‍.g.console.log(index2)
    ...
    if (turniejBlockade) {
        ...
        if ((userstate.mod || userstate.username == 'itzmaxinho') && message.toLowerCase() == `!3v3`) {
            ...
            if (turniejList.length >= 3) {
                ...
                _684‍.g.console.log(turniejList)
                turniejTeamCounter++;
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if ((message.toLowerCase().startsWith('-rep')) || (message.toLowerCase().startsWith('+rep'))) {
        ...
        _684‍.g.console.log("shall be - | command:" + command)
        _684‍.g.console.log("rest: " + rest)
        _684‍.g.console.log("argument: " + argument)
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (blockadeWord == true || self || message.match(/(.)\1{2,}/g) || message.match(/[^\wąęóźżćśłń]/g) || message.match(/\d/g)) {
        ...
        function deleteFromDB() {
            ...
            _684‍.g.console.log(dbplayers.word)
        }
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (blockadeTaczki) {
        ...
        if (message.toLowerCase().startsWith('!taczki')) {
            ...
            _684‍.g.console.log(counter)
            _684‍.g.console.log(r)
...
// !!octane
let blockadeOctane = false;

client.on('message', (channel, userstate, message, self) => {
    if (self || blockadeGlobal || blockadeOctane) return;

    if (message.toLowerCase() === `!octane`) {
        client.say(channel, `/me ` + `@${userstate['display-name']}, https://clips.twitch.tv/BlightedInexpensiveMangetoutAMPEnergyCherry-YNyOyVTLbFPHb4WP`)
        blockadeOctane = true;
        setTimeout(() => {
            blockadeOctane = false;
        }, 15000)
    }
});
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (blockadeOpen) {
        ...
        if (message.toLowerCase() === '!open') {
            ...
            client.say(channel, `/me ` + `Majster na tym streamie zmissował opena ${openCounter} raz` + openText + `! ` + highScoreText)
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (blockadeAlko) {
        ...
        if (message.toLowerCase() === `!tytul` || message.toLowerCase() === `!tytuł`) {
            client.say(channel, `/me ` + `Tytuł tego streama to: Dzień pierwszy rzucenia alkoholu... `)
            setTimeout(() => {
                client.say(channel, `/me ` + `... który na tym streamie nigdy nie nadejdzie x)`)
            }, 4000)
            blockadeAlko = true;
            setTimeout(() => {
                blockadeAlko = false;
        ...
        if (message.toLowerCase() == `!alko`) {
            client.say(channel, `/me ` + `Wierzysz temu alkoholikowi...?`)
            blockadeAlko = true;
            setTimeout(() => {
                blockadeAlko = false;
            }, 30000)
...
// !chat
let blockadeCHAT = false;

client.on('message', (channel, userstate, message, self) => {
    if (self || blockadeGlobal) return;

    if (blockadeCHAT) return;

    else {

        if (message.toLowerCase() == `!chat`)

            client.say(channel, `/me ` + `Gierka tryhard dlatego czat będzie czytany w wolnej chwili x)`)
...
// !lb
let blockadeLB = false;

client.on('message', (channel, userstate, message, self) => {
    if (self || blockadeGlobal) return;

    if (blockadeLB) return;

    else {

        if (message.toLowerCase() == `!lb`) {

            client.say(channel, `/me ` + `Leaderboardsy tutaj: https://www.reachthefinals.com/leaderboard-beta?utm_source=ingame&utm_medium=menu&utm_campaign=cb`)
...
client.on('message', (channel, userstate, message, self) => {
    ...
    async function getCurrentViewers() {
        const url = 'https://api.twitch.tv/helix/chat/chatters?broadcaster_id=streamerTwitchID&moderator_id=792294524';
        const headers = {
            ...
            if (result.greaterThan3 && userstate.username != `itzmaxinho`) {
                ...
                function Fight(viewer1, stats1, viewer2, stats2) {
                    ...
                    while ((stats1.hp > 0) && (stats2.hp > 0)) {
                        ...
                        _684‍.g.console.log(`Runda ${runda}: ${viewer1} (${stats1.hp} HP) atakuje za ${r1}` + luck1Text + `, a ${viewer2} (${stats2.hp} HP) atakuje za ${r2}` + luck2Text + `!`)
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (userstate.mod && message.toLowerCase() == `!resetlvlup`) {
        _684‍.g.console.log(currentUserLvlup)
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (userstate.mod && message.toLowerCase() == `!resetskill`) {
        _684‍.g.console.log(currentUserSkill)
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (message.toLowerCase().startsWith(`!top `) || message.toLowerCase() === `!top` || message.toLowerCase() === `!topka`) {
        ...
        const filteredViewers = dbeco.viewers.filter(viewer => viewer.nickname !== 'itzmaxinho' && viewer.nickname !== 'bot__maciek' && viewer.nickname !== 'tester' && viewer.nickname !== 'botmodwab');
        ...
        for (let i = 0; i < maxRecords; i++) {
            ...
            if (viewer.nickname !== 'itzmaxinho' && viewer.nickname !== 'bot__maciek') { // fresh topka
...
// !dungeon
let captainLevel;
let currentStage = 0; // TODO
let minLevelToEntry = 15;
let tempCaptain = []; // = [`itzmaxinho`]; //TODO
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (message.toLowerCase().startsWith(`!dungeon`) || tempCaptain.includes(userstate.username)) {
        ...
        if (tempCaptain.length == 0) {
            ...
            _684‍.g.console.log("tempCaptain: " + tempCaptain)
        ...
        if (currentStage == 2 && tempCaptain.includes(userstate.username)) {
            if (message.toLowerCase().startsWith(`!inv`) && dungeonTeammates.length <= 2) {
                ...
                if (!argument) {
                    ...
                    _684‍.g.console.log(argLvl)

                    _684‍.g.console.log("currentViewers: " + currentViewers)
        ...
        if (currentStage == 3 && (tempCaptain.includes(userstate.username) || dungeonTeammates.includes(userstate.username))) {
            ...
            let goldLeft = getGoldByNickname(userstate.username)
            _684‍.g.console.log("itemsToBuyDung: " + itemsToBuyDung)
        ...
        if (currentStage == 4 && tempCaptain.includes(userstate.username)) {
            ...
            function dungeonFight() {
                ...
                _684‍.g.console.log(bronPower)
                _684‍.g.console.log(kevPower)
                _684‍.g.console.log(koniczynaPower)
                ...
                setTimeout(() => {
                    if (start) {
                        ...
                        _684‍.g.console.log("PLAYER: " + dungeonTeammates[i])
                        _684‍.g.console.log("currentHP: " + tempHerosHp[i])
                        _684‍.g.console.log("dungHP: " + dungHP)
                        ...
                        _684‍.g.console.log(`${dungBoss} (${dungHP} hp) atakuje za ${atak}` + luckText + ` i pozostawia ${usernameToDisplayName(dungeonTeammates[i])} ${currentHP[dungeonTeammates[i]]} hp!`)
                        ...
                        _684‍.g.console.log("PLAYER: " + dungeonTeammates[i])
                        _684‍.g.console.log("tempHerosHp[i]: " + tempHerosHp[i])
                        _684‍.g.console.log("dungHP: " + dungHP)
                        dungHP = dungHP - atakFixed;

                        //client.say(channel, `/me ` + `${hero} (${heroStats.hp} hp) atakuje za ${atak} i pozostawia ${boss} ${dungHP} hp!`)
                        _684‍.g.console.log(`${usernameToDisplayName(dungeonTeammates[i])} (${tempHerosHp[i]} hp) atakuje za ${atak}` + luckText + ` i pozostawia ${dungBoss} ${dungHP} hp!`)
                    ...
                    if (currentHP[dungeonTeammates[i]] <= 0) {
                        ...
                        if (valueUlt == dungeonTeammates[i]) {
                            ...
                            _684‍.g.console.log(r + ifHadUltText + ifHadBronText + ifHadKevText + ifHadKoniczynaText)

                            // const index = dungeonTeammates.indexOf(dungeonTeammates[i]);
                            // if (index > -1) dungeonTeammates.splice(index, 1);

                            _684‍.g.console.log("isDead: " + isDead)
...
client.on('message', (channel, userstate, message, self) => {
    ...
    // console.log(`to jest ${cos}, a to juz jest ${usernameToDisplayName(cos)}`)
    //- - --  - - - -- - - - - - - - - - - - - - - - - - - -- - - - - - - 
    ...
    // console.log([atak, hape, lucky])
    // atak1 += atak
    // hape1 += hape
    // lucky1 += lucky
    // countterr++

    // console.log(countterr + `. ATK: ` + atak1 + ` | HP: ` + hape1 + ` | LUCK: ` + lucky1)

    //  --- - --  -  -- - --  - - - - -- - -- - - --  - - - -- - - - - - - - - - - - - - - - - - - -- - - - - - - 

    //     // get bosses defeated by nickname username
    //     function getNumbersMason(username) {
    //         const lol = dbtime.extraNumbers.find(a => a.nickname === username);
    ...
    //         }
    //     }
    //     let numberki = getNumbersMason(userstate.username)
    //     console.log(numberki)
    //     if (numberki.includes("722")) {
...
function TimeoutUser(nickname, duration) {
    ...
    fetch(`https://api.twitch.tv/helix/moderation/bans?broadcaster_id=streamerTwitchID&moderator_id=792294524`, {
        method: 'POST',
        headers: {
            'Authorization': `Bearer ${TOKEN}`,
            'Client-Id': ClientID,
...
function WhisperUser(nickname, message) {
    ...
    fetch(`https://api.twitch.tv/helix/whispers?from_user_id=792294524&to_user_id=${user_id}`, {
        method: 'POST',
        headers: {
            'Authorization': `Bearer ${TOKEN}`,
            'Client-Id': ClientID,
...
function UnbanBlackten() {

    fetch(`https://api.twitch.tv/helix/moderation/bans?broadcaster_id=streamerTwitchID&moderator_id=792294524&user_id=162773860`, {
        method: 'DELETE',
        headers: {
            'Authorization': `Bearer ${TOKEN}`,
            'Client-Id': ClientID
        }
    }).then(res => _684‍.g.console.log("blackzjeb: " + res.status + " | 400 - nie jest zbanowany, 200 - był zbanowany"))
        .catch(err => {
            _684‍.g.console.error(err);
        });
}
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (((listPanda.length == 0) && (userstate.username == 'zawistnapanda') && (message.toLowerCase().includes('arn'))) || ((listPanda.length == 0) && (userstate.username == 'zawistnapanda') && (message.toLowerCase().includes('igg')))) {
        client.say(channel, `/me ` + `O, siema @zawistnapanda MLADY`);

        listPanda.push(userstate['display-name']);
    }
});
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (((listKoko.length == 0) && (userstate.username == 'itzkoko27') && (message.toLowerCase().includes('brb')))) {
        client.say(channel, `/me ` + `A, dobra @itzKoko27 to nara dobranoc`);

        listKoko.push(userstate['display-name']);
    }
});
...
let listLucjan = [];
let optionsLucjan = [
    `@WilgotnyLucjan !!! Piekarnik nastawiłeś, zara sie wszystko zjara NotLikeThis`,
    `@WilgotnyLucjan !!! Piekarnik nastawiłeś, zara sie wszystko zjara NotLikeThis`,
    `@WilgotnyLucjan !!! Piekarnik nastawiłeś, zara sie wszystko zjara NotLikeThis`,
    `@WilgotnyLucjan, budzik nastaw na rano MLADY`,
    `@WilgotnyLucjan, budzik nastaw na rano MLADY`,
    `@WilgotnyLucjan, kiedy wolne jakieś na chlanie?`,
    `@WilgotnyLucjan, kiedy midgard?`,
];

client.on('message', (channel, userstate, message, self) => {
    if (self) return;

    if ((listLucjan.length == 0) && (userstate.username == 'wilgotnylucjan')) {
        client.say(channel, `/me ` + `Eluwina Wilgotny Maciej MLADY`);

        let r = optionsLucjan[Math.floor(Math.random() * optionsLucjan.length)]

        setTimeout(() => {
            client.say(channel, `/me ` + `${r}`)
        }, 900000);

        listLucjan.push(userstate['display-name']);
    }
});
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (((listLegate.length == 0) && (userstate.username == 'leggate') && (message.toLowerCase() == '!los'))) {
        client.say(channel, `/me ` + `Wstrętna choroba ten !los, co @${userstate['display-name']}? KEKW Kiedyś się uda!`);

        listLegate.push(userstate['display-name']);
    }
});

// TESTING ://0.0.5
let pandzioBlock = false

client.on('message', (channel, userstate, message, self) => {
    if (self) return;

    if ((!pandzioBlock && userstate['user-id'] == 131390717 && message.includes('itzmax3SOK'))) {
        client.say(channel, `/me ` + `Soczek do mordy jest lany MLADY`);
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (((listRybak.length == 0) && (userstate.username == 'polski_rybak') && ((message.includes('zdrowko')) || (message.includes('zdrowko'))))) {
        client.say(channel, `/me ` + `Ooo @${userstate['display-name']}, bak na 200 ustawiony, ale na chlanie KEKW zdrówko!`);

        listRybak.push(userstate['display-name']);
    }
});
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (((listJacobs.length == 0) && (userstate.username == 'ajmjacobs') && (message == '!smacznejkawusi'))) {
        client.say(channel, `/me ` + `@${userstate['display-name']} wiadomo co i kogo EZ`);

        listJacobs.push(userstate['display-name']);
    }
});
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (((listQtaz.length == 0) && (userstate.username == 'queuetaz') && (message.toLowerCase().includes("kappa")))) {
        client.say(channel, `/me ` + `Oho hihi śmieszek się obudził Kappa`);

        listQtaz.push(userstate['display-name']);
    }
});
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (((userstate.username == 'azza__rl') && (message.toLowerCase().includes("!azza")))) {
        client.say(channel, `/me ` + `Blunder. Hung my queen, missed mate in 1 NotLikeThis`);
    }
});
...
let oWyChujeList = [
    `O wy chuje KEKW`,
    `Ale że tak sam siebie KEKW`,
    `Kurrwica, znowu to samo KEKW`,
    `Który to wykupił dziad KEKW`,
    `Ja mam immunitet ptaszku MLADY`,
];

client.on('message', (channel, userstate, message, self) => {

    if (((listNightbot.length == 0) && (userstate.username == 'nightbot' || userstate.username == 'bot__maciek') && (message.toLowerCase().includes("powerupl bot__maciek")))) {
        let randomAnswer = oWyChujeList[Math.floor(Math.random() * oWyChujeList.length)]

        setTimeout(() => {
            client.say(channel, `/me ` + randomAnswer);
        }, 2000);

        listNightbot.push(userstate['display-name']);
    }
});
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (((listNurse2.length == 0) && (userstate.username == 'itznursiakk') && (message.toLowerCase().includes("gnoje")))) {
        client.say(channel, `/me ` + `siema gnoju peepoLove`);

        listNurse2.push(userstate['display-name']);
    }
});
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (((listKelzi.length == 0) && (userstate['user-id'] == 104174782))) {

        client.say(channel, `/me ` + `Ebe Ebe? Czy to Ty? oKurwa Witam arcymistrza szachowego MLADY`);

        listKelzi.push(userstate['display-name']);
    }
});
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if (((listitzCurser.length == 0) && (userstate['user-id'] == 778123454))) {

        client.say(channel, `/me ` + `Chat, Baczność!`);

        listitzCurser.push(userstate['display-name']);
    }
});
...
client.on('message', (channel, userstate, message, self) => {
    ...
    if ((!itzIgnore.includes(userstate.username)) && (!itzIgnore.includes(userstate['user-id'])) && (!listOfItz.includes(userstate['display-name'])) && (userstate.username.toLowerCase().startsWith('itz'))) {
        client.say(channel, `/me ` + `@${userstate['display-name']} Witam koxa najlepszego, szacun na dzielni mordziu MLADY`);

        listOfItz.push(userstate['display-name'])
    }
});